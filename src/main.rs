...
fn is_viewable(conn: &xcb::Connection, window: xcb::Window) -> bool {
    let attrs = xcb::get_window_attributes(conn, window)
        .get_reply()
        .unwrap();
    (attrs.map_state() & xcb::MAP_STATE_VIEWABLE as u8) != 0
}

fn point_in_window_geom(
    conn: &xcb::Connection,
    window: xcb::Window,
    x: u32,
    y: u32,
) -> Option<(u32, i16, i16, u16, u16)> {
    let geom = xcb::get_geometry(conn, window).get_reply().unwrap();
    let (gx, gy, gw, gh): (i16, i16, u16, u16) = (geom.x(), geom.y(), geom.width(), geom.height());
    if contained(
        geom.x(),
        geom.y(),
        geom.width() as i16,
...
fn get_window_at_point(conn: &xcb::Connection, win: xcb::Window, x: u32, y: u32) {
    ...
    let (child, gx, gy, gh, gw) = tree
        .children()
        .iter()
        .filter(|&&child| is_viewable(conn, child))
        .filter_map(|&child| point_in_window_geom(conn, child, x, y))
        .nth(1)
        .expect("No viewable window at point!");
